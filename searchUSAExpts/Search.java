...
public abstract class Search {
  ...
  protected SearchNode oldNode; // node found on open with same state as new one
  ...
  public String runSearch(SearchState initState, String strat) {
    ...
    while (!open.isEmpty()) {
      ...
      /*
       * print contents of open System.out.println("-------------------------");
       * System.out.println("iteration no "+cnum); System.out.println("open is"); for
       * (SearchNode nn: open) { String nodestr=nn.toString();
  ...
  /**
   * runSearchE runSearch without printout returns efficiency - 0 for failure
   * 
   * @param initState initial state
   * @param strat     - String specifying strategy
   * @return indication of success or failure
  ...
  private void vetSuccessors() {
    ...
    for (SearchNode snode : successorNodes) {
      if (onOpen(snode)) { // on open - usual DP check
        if (snode.getGlobalCost() <= oldNode.getGlobalCost()) {
          oldNode.setParent(snode.getParent()); // better route, modify node
          oldNode.setGlobalCost(snode.getGlobalCost());
          oldNode.setLocalCost(snode.getLocalCost());
          oldNode.setestTotalCost(snode.getestTotalCost());
        }
      } else {
        if (onClosed(snode)) { // A* - on closed - DP check again
          if (snode.getGlobalCost() <= oldNode.getGlobalCost()) {
            ...
            oldNode.setLocalCost(snode.getLocalCost());
            oldNode.setestTotalCost(snode.getestTotalCost());
            open.add(oldNode); // A* - add the node back to open
            closed.remove(oldNode); // A* - & remove it from closed
          }
        } else
          vslis.add(snode); // not seen before
      }
    }
  ...
  // A* - if node found, remember it in oldNode
  ...
  // if node found, remember it in oldNode
